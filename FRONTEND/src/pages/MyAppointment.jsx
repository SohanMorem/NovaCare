...
import axios from "axios";
import { loadStripe } from "@stripe/stripe-js";
import "../styles/MyApppointment.css";  // Importing the CSS file
  ...
  const paymentIntegration = async (appointmentId) => {
    try {
      ...
      const response = await axios.post(
        backendurl + "/api/user/paymentintegration",
        { appointmentId },
        { headers: { token } }
      );
          ...
          <div key={index} className="appointment-card">
            ...
            <div className="doctor-info">
              ...
              <div className="doctor-details">
                <h2>{appointment.docData.name}</h2>
                <p><span>{appointment.docData.speciality}</span></p>
                <p><span>Address:</span> {appointment.docData.address.line1}, {appointment.docData.address.line2}</p>
                <p><span>Date & Time:</span>Date & Time: {slotDateFormat(appointment.slotDate)} | {appointment.slotTime}</p>
              </div>
            </div>
            ...
            <div className="actions">
              {!appointment.isCompleted && (
                <>
                  {!appointment.cancelled && !appointment.payment && (
                    <button className="pay-amount-btn" onClick={() => paymentIntegration(appointment._id)}>
                      Pay Amount
                    </button>
                  )}
                  {appointment.payment && !appointment.cancelled && (
                    <button className="payment-done-btn">Payment Done</button>
                  )}
                  {!appointment.cancelled && (
                  ...
                  {appointment.cancelled && (
                    <button className="canceled-btn">Canceled</button>
                  )}
                </>
              )}
              {appointment.isCompleted && (

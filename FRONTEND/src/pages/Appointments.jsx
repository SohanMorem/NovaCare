...
const Appointments = () => {
  ...
  const getAvailableSlots=async ()=>{
    ...
    for(let i=0;i<7;i++){
      ...
      if(today.getDate()===currentDate.getDate()){  // if today date and current date equal then only
  ...
  return docInfo && (
    <div>
        ...
        <div className='flex-1 border border-gray-500 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
          ...
          <p className='text-gray-900 font-medium mt-4'>Appointment Fee: 
            <span className='text-black'>{currencySymbol}{docInfo.fees}</span></p>
        </div>

        </div>

        {/* slot for Appointmnet */}
            ...
            <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
              {
                docSlots.length && docSlots.map((item,index)=>(
                  <div onClick={()=>setSlotIndex(index)} className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${ slotIndex === index  ? ' bg-primary text-white ' : 'border border-gray-300 text-black' }`} key={index}>
            ...
            <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
              {
                docSlots.length && docSlots[slotIndex].map((item,index)=>(
                  <p onClick={()=>setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${ item.time === slotTime ?'bg-primary text-white':'text-black border border-gray-300'}`} key={index}>
